...
function getCube() {
  ...
  product_details.forEach(function (i) {
    ...
    let bill = [];
    const product = product_details.find((p) => p.prdid == uprod_id);
    if (!product) {
      alert("product not found in list");
    }
    let existingProduct = checkArray.find((p) => p.prdid == uprod_id);
    if (existingProduct) {
      existingProduct.quantity += upqnos;
      res = upqnos * existingProduct.price;
      existingProduct.price += res;
    } else {
      if (checkArray.length >= 5) {
        alert("max product reached");
        return;
      }
      existingProduct = { ...product, quantity: quantity, price: price };
      checkArray.push(existingProduct);
      console.log(checkArray);
    }
